...
#Load defaults
SCRIPT_DIR=/home/users/lbennett/proc_test/
source $SCRIPT_DIR/chilbolton_defaults.cfg
...
lotus_outdir=$LOTUS_OUTPUTS_BASEDIR/chilbolton/$YYYY/$MM/$DD
...
print_cmd="sbatch -p $QUEUE -t $wallclock -o $lotus_outdir/${logfile}.out -e $lotus_outdir/${logfile}.err --wrap=\"time $SCRIPT_DIR/calibrate_chilbolton_chunk.sh\""
echo "[INFO] Running: $print_cmd"

script_cmd=$(sbatch -p $QUEUE -t $wallclock -o $lotus_outdir/${logfile}.out -e $lotus_outdir/${logfile}.err --wrap="time $SCRIPT_DIR/calibrate_chilbolton_chunk.sh ${ARGS}")
echo $script_cmd
