...
while [[ $chunk -lt 24 ]]; do
    ...
    wallclock=$(printf %02d $((${CHUNK_SIZE} / 2))):00:00 
    
    output_base=$output_basedir/$(echo $ARGS | cut -d' ' -f3)

    
    print_cmd="sbatch -p $QUEUE -t $wallclock -o ${output_base}.out -e ${output_base}.err --wrap=\"time $SCRIPT_DIR/convert-raine-x-band-hour.sh ${ARGS}\""
    echo "[INFO] Running: $print_cmd"

    script_cmd=$(sbatch -p $QUEUE -t $wallclock -o ${output_base}.out -e ${output_base}.err --wrap="time $SCRIPT_DIR/convert-raine-x-band-hour.sh ${ARGS}")
    echo $script_cmd
